...
static NSString *featureAId = @"com.electric.handbook.iap1";
...
- (void) requestProductData
{
    ...
    //myTimer = [NSTimer scheduledTimerWithTimeInterval:15.0f
                                               //target:self selector:@selector(tick) userInfo:nil repeats:NO];
}

- (void)tick {

    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Message" message:@"Your enternet connection is too slow. Please try again later." delegate:self cancelButtonTitle:@"Close" otherButtonTitles:nil];
    [alert show];
