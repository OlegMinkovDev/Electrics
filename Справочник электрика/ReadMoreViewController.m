...
@synthesize readMoreWebView;

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    readMoreWebView.delegate = self;
    readMoreWebView.translatesAutoresizingMaskIntoConstraints = YES;
    
    // Получаем ссылку на AppDelegate
    AppDelegate *delegate = (AppDelegate*)[[UIApplication sharedApplication] delegate];
    
    // Загружаем страницу
    NSURLRequest *localRequest = [NSURLRequest requestWithURL: [NSURL fileURLWithPath:delegate.onlineHelpLocalFilePath]];
    [self SetCorrectSizeForComponent];
    [readMoreWebView loadRequest:localRequest];
    
    self.navigationItem.title = @"Справка online помощника";
...
- (void)SetCorrectSizeForComponent {
    ...
    [readMoreWebView setFrame:correctFrame];
}
