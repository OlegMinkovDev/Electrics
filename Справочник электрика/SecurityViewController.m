...
@synthesize securityWebView;

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    securityWebView.delegate = self;
    securityWebView.translatesAutoresizingMaskIntoConstraints = YES;
    
    // Получаем ссылку на AppDelegate
    AppDelegate *delegate = (AppDelegate*)[[UIApplication sharedApplication] delegate];
    
    // Загружаем страницу
    NSURLRequest *localRequest = [NSURLRequest requestWithURL: [NSURL fileURLWithPath:delegate.electricanSecurityLocalFilePath]];
    [self SetCorrectSizeForComponent];
    [securityWebView loadRequest:localRequest];
    
    self.navigationItem.title = @"Основы безопасности";
...
- (void)SetCorrectSizeForComponent {
    ...
    [securityWebView setFrame:correctFrame];
}
